...
| 第 2 步 | `专线网卡 · 移动入口 (China Mobile)` | 原样复制移动入口 IP。不要填写“专线网卡 · 外部连接 IP”。 |
...
- 自动生成用户名和密码，并输出兼容节点面板的 `mieru://` 协议链接。
...
如果系统正在执行自动更新，脚本会等待 `apt/dpkg` 软件包锁释放，最长 300 秒，然后自动继续。请勿手动删除 `/var/lib/dpkg/` 下的锁文件。
